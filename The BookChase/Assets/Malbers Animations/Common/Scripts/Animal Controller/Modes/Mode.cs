    ...
    public class Mode
    {
        ...
        public virtual bool TryActivate()
        {
            ...
            if (Animal.IsPlayingMode)              // Means the there's a Mode Playing
            {
                ...
                else
                {
                    ...
                    if (!ActiveMode.InCoolDown)   //Means that the Active mode can be Interrupted since is no longer on cooldown
                    {
                        ActiveMode.ResetMode();
        ...
        public void AnimationTagEnter(int animatorTagHash)
        {
            ...
            if (ActiveAbility != null)
            {
                ...
                if (Animal.debugModes) Debug.Log($"Mode: <B>{Name}<color=Green> [ANIMATION ENTER] </color></B>  with Ability: <B> {ActiveAbility.Name} [{CurrentPlayingAnimation}] </B> [{AMode}]  Time: " + Time.time.ToString("F3"));

                if (HasCoolDown)
                    Animal.StartCoroutine(C_SetCoolDown(CoolDown));

                Animal.SetModeStatus(IntID);
            }
        }
